...
router.get("/dashboard/stats", async (req, res): Promise<void> => {
  ...
  const completedAssessments = await db.select().from(assessmentsTable)
  ...
  for (const p of patients) {
    conditionMap[p.primaryCondition] = (conditionMap[p.primaryCondition] ?? 0) + 1;
  }
  ...
  res.json({
    ...
    orangeAlertCount,
    greenCount,
    averageGlobalScore: scoredCount > 0 ? Math.round(totalScore / scoredCount) : 0,
    conditionBreakdown,
    recentAssessments: assessments,
  });
});

router.get("/risk-summary", async (req, res): Promise<void> => {
